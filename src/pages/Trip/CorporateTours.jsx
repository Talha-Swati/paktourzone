        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {tourData.features.map((feature, index) => (
              <div key={index} className={`p-6 rounded-xl border-2 ${isDarkMode ? 'bg-[#141A1F] border-[#1E242B] hover:border-[#22D3EE]' : 'bg-white border-[#E2E8F0] hover:border-[#2563EB]'} transition-all`}>
                <Check className={`w-6 h-6 ${isDarkMode ? 'text-[#22D3EE]' : 'text-[#2563EB]'} mb-2`} />
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {tourData.packages.map((pkg) => (
              <div key={pkg.id} className={`rounded-2xl overflow-hidden shadow-xl ${isDarkMode ? 'bg-[#141A1F]' : 'bg-white border border-[#E2E8F0]'} `}>
                ...
                <div className="p-6">
                  <h3 className={`text-2xl font-bold mb-2 ${isDarkMode ? 'text-white' : 'text-[#0F172A]'}`}>{pkg.name}</h3>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-6">
                    <div className="flex items-center gap-2">
                      ...
                      <div><p className="text-xs text-[#94A3B8]">Duration</p><p className={`font-semibold ${isDarkMode ? 'text-white' : 'text-[#0F172A]'}`}>{pkg.duration}</p></div>
                    </div>
                    <div className="flex items-center gap-2">
                      <Users className={`w-5 h-5 ${isDarkMode ? 'text-[#22D3EE]' : 'text-[#2563EB]'} `} />
                      <div><p className="text-xs text-[#94A3B8]">Group Size</p><p className={`font-semibold ${isDarkMode ? 'text-white' : 'text-[#0F172A]'}`}>{pkg.groupSize}</p></div>
                    </div>
                  </div>

                  <div className="mb-6">
                    <h4 className={`font-semibold mb-3 ${isDarkMode ? 'text-[#22D3EE]' : 'text-[#2563EB]'} `}>Key Activities</h4>
                  ...
                  <div className={`border-t pt-4 mb-4 ${isDarkMode ? 'border-gray-700' : 'border-[#E2E8F0]'}`}>
                    <div className="flex items-baseline gap-2 mb-2">
                      ...
                      <span className="text-sm text-[#94A3B8]">per person</span>
                    </div>
                  </div>
